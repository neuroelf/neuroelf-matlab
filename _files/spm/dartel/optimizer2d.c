...
static void restrict_(int n, int na[], double *a, int nc[], double *c, double *b)
{
    int i;
    for(i=0; i<n; i++)
    {
          ...
          double *u0, double *scratch)
{
    ...
    for(j=1; j<ng; j++)
    {
        ...
        restrict_(2,n[j-1],bo[j-1],n[j],bo[j],rbuf);
        restrict_(3,n[j-1],a[j-1],n[j],a[j],rbuf);
    }
    ...
    for(j=ng-2; j>=0; j--)
    {
        ...
        for(jc=0; jc<c; jc++)
        {
            ...
            for(jj=j; jj<ng-1; jj++)
            {
                ...
                restrict_(2,n[jj],res,n[jj+1],b[jj+1],rbuf);
