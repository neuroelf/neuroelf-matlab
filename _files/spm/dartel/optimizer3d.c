...
static void restrict_(int n,  int na[], float *a,  int nc[], float *c, float *b)
{
    int i;
    for(i=0; i<n; i++)
    {
...
#   endif
    ...
    for(j=1; j<ng; j++)
    {
        restrict_(3,n[j-1],bo[j-1],n[j],bo[j],rbuf);
        restrict_(6,n[j-1],a[j-1],n[j],a[j],rbuf);
        ...
        for(j=ng-2; j>=0; j--)
        {
            ...
            for(jc=0; jc<c; jc++)
            {
                ...
                for(jj=j; jj<ng-1; jj++)
                {
                    ...
                    for(i=0; i<3*m[jj]; i++)
                        res[i] = b[jj][i] - res[i];
                    restrict_(3,n[jj],res,n[jj+1],b[jj+1],rbuf);
    ...
    else
    {
        int jc;
        for(j=1; j<ng; j++)
            restrict_(3,n[j-1],u[j-1],n[j],u[j],rbuf);
