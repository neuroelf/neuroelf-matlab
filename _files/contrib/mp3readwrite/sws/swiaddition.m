...
%Default to add all oscillators
if isempty(swave)
	swave=1:nOscs(1,1);
end

adder=str2num(sw_out{3,:});
...
if isequal('F',tbm) | isequal ('f', tbm)
	for x=1:size(swave,2)
		F(swave(1,x),timer(1,1):timer(end,1))=F(swave(1,x),timer(1,1):timer(end,1)) + adder;
	end
elseif isequal('A',tbm) | isequal ('a', tbm)
	for x=1:size(swave,2)
		M(swave(1,x),timer(1,1):timer(end,1))=M(swave(1,x),timer(1,1):timer(end,1)) + adder;
	end
elseif isequal('T',tbm) | isequal ('t', tbm)
	timeslice(timer(1,1):timer(end,1),:)=timeslice(timer(1,1):timer(end,1),:) + adder;
else
	errordlg('Can only add value to Frequency, Amplitude, or Time', '');
	return;
end;
