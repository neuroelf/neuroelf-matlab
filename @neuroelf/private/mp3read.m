...
p = [neuroelf_path('contrib') filesep 'mp3readwrite'];
mpg123 = fullfile(p,['mpg123.',ext]);
mp3info = fullfile(p,['mp3info.',ext]);

%%%%% Check for network mode
if length(FILE) > 6 && (strcmpi(FILE(1:7),'http://') || strcmpi(FILE(1:6),'ftp://'))
...
if ~OVERNET
  ...
  starpos = find(w == '*');
  nums = str2num(w(1:(starpos(1) - 2)));
  strs = tokenize(w((starpos(1)+2):end));
...
w = w((1+max([0,findfirst(w == 10)])):end);
% Debug
%disp([cmd,' -> ','*',w,'*']);
